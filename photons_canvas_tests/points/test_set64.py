...
describe "set64":
    ...
    it "allows you to create directly with attributes":
        colors = [{"hue": i, "saturation": 1, "brightness": 1, "kelvin": 3500} for i in range(64)]
        ...
        real = TileMessages.Set64.empty_normalise(**kwargs)
        simple = Set64(**kwargs)
    ...
    it "can be given colors in multiple ways", seed:
        ...
        setting = [c1, c2]
        real = seed.clone()
        real.colors = original

        simple = Set64(colors=setting)
        assert real.pack() == simple.pack()
        assert simple.colors == [Color(*c1), Color(*c2)]

        setting = [c1, c2] * 3
        simple = Set64(colors=setting)
        assert simple.colors == [Color(*c1), Color(*c2)] * 3
