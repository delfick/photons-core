...
class RetryOptions:
    ...
    async def tick(self, timeout, min_wait=0.1, get_now=time.time):
        timeouts = list(self.timeouts)
        step, end = timeouts.pop(0)
        ticker = hp.ATicker(max_time=timeout, every=step)
        ...
        async for _ in ticker:
            ...
            if end and now - last > end:
                if timeouts:
                    step, end = timeouts.pop()
                    self.change_every(step)
                else:
                    end = None
            ...
            yield final_time - time.time(), ticker.diff
