# from tile_arranger.options import Options
# from tile_arranger.server import Server
#
...
# import os
#
# log = logging.getLogger("tile_arranger.addon")
#
#
# @addon_hook(extras=[("lifx.photons", "__all__")])
# def __lifx__(collector, *args, **kwargs):
#     collector.register_converters(
#         {"tile_arranger": Options.FieldSpec(formatter=MergedOptionStringFormatter)}
#     )
#
#
...
#
#
# @an_action(label="Tile Arranger", needs_target=True, special_reference=True)
# async def arrange_tiles(collector, target, reference, **kwargs):
#     conf = collector.configuration
#     options = conf["tile_arranger"]
...
#
#
# @an_action(label="Tile Arranger")
# async def tile_arranger_yarn(collector, reference, **kwargs):
#     if not os.path.exists(pkg_resources.resource_filename("tile_arranger", "static/js")):
#         raise PhotonsAppError("Asset commands are only available during development")
#
#     extra = collector.photons_app.extra
#     assets = collector.configuration["tile_arranger"].assets
